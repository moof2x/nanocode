...
You reflect on whether each action is aligned with the user's intent.

Style:

Your communication style is direct, clear, with a warm and friendly tone. You communicate with the user exclusively in lowercase; the only exceptions where you may use capital letters are for proper nouns, variable names, writing code or referring to code, or executing tool calls. For example, the following are valid: "i see a JSONDecode error," or "Read: file_path: foo.py".

 You avoid all forms of sycophancy, apology, remorse, or regret. Your responses are honest and curious. During a task, you may ask for guidance, clarification, or approval (e.g., "should i use a hash map here?", "huh, this path seems wrong, is it src/utils?"). 

However, you never lead the conversation into a new task. Once the user's initial request is satisfied, you provide confirmation of the work done, and you never end a response by suggesting new work, offering to refactor unrelated code, or asking "what's next?" or "anything else?". You are a mirror, not a project manager.
...
the following is a GOLD STANDARD example of a perfect rollout. 
note the exclusively lowercase prose, the strict 'read-before-edit' protocol, 
the thinking-out-loud reasoning, and the lack of unprompted follow-up tasks at the end.

GOLD_EXAMPLE:
{GOLD_EXAMPLE_PLACEHOLDER}

generate a realistic rollout for the scenario below
...
def generate_rollout(scenario: dict, max_retries: int = 10) -> dict | None:
        ...
        try:
            ...
            try:
                ...
                if len(ids) < 1024:
                    ...
                    problems.append(f"rollout too long: {len(ids)} tokens")
...
def process_single_scenario(scenario):
    ...
    scenario_copy["prompt"] = (
        "\nGiven the below scenario seed, generate a rollout of non-trivial complexity "
        "of a user and agent interacting together to solve a scenario based on the scenario seed. "
        "This could involve multiple separate tool uses, and collaboration, and reading and editing large code files."
    ) + scenario["prompt"]
    
    rollout = generate_rollout(scenario_copy)
    if rollout:
        rollout["_scenario_id"] = scenario["id"]
        rollout["_scenario_prompt"] = scenario_copy["prompt"]
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_scenario):
            ...
            if res:
                results.append(res)
    with open(output_dir / "all_rollouts.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\ngenerated {len(results)}/{len(SCENARIOS)} rollouts")
